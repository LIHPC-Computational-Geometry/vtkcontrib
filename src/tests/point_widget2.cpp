...
class vtkConstrainedPointWidget2Callback : public vtkCommand
{
	public :

	static vtkConstrainedPointWidget2Callback* New ( )
	{ return new vtkConstrainedPointWidget2Callback ( ); }

	virtual void SetWidget (vtkConstrainedPointWidget2* w)
	{ Widget	= w; }
	...
	vtkConstrainedPointWidget2Callback ( )
		: vtkCommand ( ), Widget (0)
	{ }
	vtkConstrainedPointWidget2Callback (const vtkConstrainedPointWidget2Callback&)
		: vtkCommand ( ), Widget (0)
	{ }
	vtkConstrainedPointWidget2Callback& operator = (const vtkConstrainedPointWidget2Callback&)
	{ return *this; }
	virtual ~vtkConstrainedPointWidget2Callback ( )
	{ }

	vtkConstrainedPointWidget2*	Widget;
};	// class vtkConstrainedPointWidget2Callback


void vtkConstrainedPointWidget2Callback::Execute (vtkObject* caller, unsigned long event, void* data)
{
	assert (0 != Widget);
	vtkConstrainedPointHandleRepresentation2*	rep	= Widget->GetConstrainedPointRepresentation ( );
	assert (0 != rep);
	double	pos [3]	= { 0., 0., 0. };
	rep->GetWorldPosition (pos);
	cout << "vtkConstrainedPointWidget2Callback::Execute. New pos ("
	     << pos [0] << ", " << pos [1] << ", " << pos [2] << ")" << endl;
}	// vtkConstrainedPointWidget2Callback::Execute
...
//	handleWidget->GetRepresentation ( )->SetHandleSize (30);
	vtkSmartPointer<vtkConstrainedPointWidget2Callback>	posCallback	= vtkSmartPointer<vtkConstrainedPointWidget2Callback>::New ( );
